...
now = datetime.now()
base_results_path = Path(f"./results/experimentB/{now.strftime('%y-%m-%d_%H-%M-%S')}")
base_results_path.mkdir(parents=True, exist_ok=True)

def generate_network(layers_count, input_channels = 3, hidden_channels = 32, output_channels = 3, kernel_size = 5, padding = 2, pool_every: int = 10):
    """
    Generate a network with the given number of layers and channels
    """

    #TODO add pooling to obtain frame level features
    ...
    for i in range(layers_count-3):
        if i % pool_every == 0:
            network.add_module(f"pool_{i}", torch.nn.MaxPool2d(kernel_size=2, stride=2))
        else:
    ...
    network.add_module("adaptive_pool", torch.nn.AdaptiveAvgPool2d(1))
...
def subexperiment(resolution_option, output_option):
    ...
    net = generate_network(layers, input_channels, hidden_features, output_channels, kernel_size, padding, pool_every=50)
    net.to(devices[0])
